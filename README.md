...
2. ضع مفتاح OpenCode في `OPENCODE_API_KEY`، ثم شغّل `npm run audit:init` لتوليد مفتاح HMAC عشوائي مستقل (32 بايت) داخل `.env.local` من دون طباعته. يكتب الأمر ملفًا مؤقتًا متزامنًا ثم يستبدل الملف ذريًا، ويرفض استبدال مفتاح موجود. يستعيد تلقائيًا قفل تهيئة أقدم من عشر دقائق؛ لا تحذف قفلًا أحدث يدويًا قبل التأكد من عدم وجود عملية `audit:init` عاملة. على Windows سيظهر تحذير ثابت لأن صلاحيات POSIX لا تتحقق من ACL؛ راجع خصائص أمان `.env.local` يدويًا واقصر القراءة على الحساب المقصود والمديرين الموثوقين. لا ترسل أيًا من المفتاحين إلى الشات أو تضعه في متغير يبدأ بـ`VITE_`.
