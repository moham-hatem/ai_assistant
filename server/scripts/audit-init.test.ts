...
import { initializeSecurityAuditKey } from './audit-init.ts';
...
test('audit:init creates .env.local without printing the generated secret', async (context) => {
  ...
  await initializeSecurityAuditKey({ cwd: directory, generateKey: () => knownBytes, log: (message) => messages.push(message) });
...
test('audit:init replaces an empty placeholder and preserves Windows line endings', async (context) => {
  ...
  await initializeSecurityAuditKey({ cwd: directory, generateKey: () => knownBytes, log: () => undefined });
...
test('audit:init refuses to overwrite an existing key', async (context) => {
  ...
  await assert.rejects(
    initializeSecurityAuditKey({ cwd: directory, generateKey: () => knownBytes, log: () => undefined }),
