...
export function TransitionDialog({ busy, copy, onCancel, onConfirm, pending }: TransitionDialogProps) {
  ...
  return (
    <div className="books-dialog-backdrop" onMouseDown={onCancel} role="presentation">
      <section aria-modal="true" className="books-dialog" onMouseDown={(event) => event.stopPropagation()} role="dialog">
        <header><span><AlertCircle size={21} /></span><div><small>{copy.statuses[pending.edition.status]} → {copy.statuses[pending.targetStatus]}</small><h2>{copy.confirmTitle}</h2></div><button aria-label={copy.cancel} onClick={onCancel} type="button"><X size={18} /></button></header>
